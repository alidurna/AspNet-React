 ...
 * 
 * Alt görev oluşturma ve düzenleme modal'ı.
 */

import React, { useState, useEffect, useRef } from 'react';
import { FaTimes, FaSave, FaCalendarAlt, FaListUl } from 'react-icons/fa';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const taskData = {
        ...
        parentTaskId: parentTask.id, // Alt görev için parent task ID'si
      };

      await onSave(taskData);
      // Modal'ı kapatma - SubTasksModal'da kalacak
    } catch (error) {
      console.error('Alt görev kaydedilirken hata:', error);
    } finally {
          ...
          <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
                {subTask ? 'Alt Görevi Düzenle' : 'Yeni Alt Görev'}
              </h2>
              <div className="flex items-center gap-2 mt-2">
                <FaListUl className="w-4 h-4 text-purple-500" />
                <span className="text-sm text-gray-600 dark:text-gray-400">
                  Ana Görev: {parentTask.title}
                </span>
              </div>
            </div>
            <button
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Alt görev açıklamasını girin"
                />
              </div>
            ...
            <div className="flex items-center justify-end gap-3 mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="px-6 py-3 bg-purple-500 hover:bg-purple-600 text-white rounded-lg font-semibold transition-colors duration-200 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? (
                  <>
                  ...
                  <>
                    ...
                    {subTask ? 'Güncelle' : 'Alt Görev Oluştur'}
                  </>
                )}
              </button>
            </div>
